"""
Fri, Apr 7 2023: Code for my paper on Compressed Sensing x Rigidity Theory
Fri, Jun 30 2023: Re-organized and added methods for generating monte carlo plots
------------------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    for params in tqdm(PARAM_SETS):
        ...
        for simulation_no in range(params["num_mc"]):
            FAULTY_DRONES = [str(number+1) for number in random.sample(range(len(POSITIONS)), 
                                                                       params["num_faulty_drones"])]
            ...
            try:
                for _ in range(params["num_scp_iterations"]):
                    ...
                    for i in range(len(G.vertices)):
                        objective_function += cp.atoms.norm(x_k[i*3:(i+1)*3].T[0] + x[i*3:(i+1)*3])

                    problem = cp.Problem(cp.Minimize(objective_function), [cp.atoms.norm(R @ (x) - z) <= tol] )
                    problem.solve()
