...
def compare_with_ekf(bandwidth, monte_carlo=False):
    ...
    if not monte_carlo:
        ...
        SIZE = 0.75
        plt.figure(figsize=(6.3*SIZE, 5.8*SIZE), dpi=1000)
        axis = plt.gca()
        G.draw(axis=axis)
        G.draw(nodelist=ekf.beacon_agents, edgelist=[], node_color='#650000', node_shape='s', axis=axis)
        PLOT_LIM_0 = PLOT_LIM
        plt.xlim(OFFSET[0]-PLOT_LIM_0, OFFSET[0]+PLOT_LIM_0)
        plt.ylim(OFFSET[1]-PLOT_LIM_0, OFFSET[1]+PLOT_LIM_0)
        ...
        plt.figure(figsize=(5.5*SIZE, 5.0*SIZE), dpi=1000)
        G.draw()
    ...
    if monte_carlo:
        ...
        print(new_labels)
        graph_bandwidth = 0
        for v in G_shuffled.vertices:
            for v_2 in G_shuffled.edges[v]:
                graph_bandwidth = max(int(v) - int(v_2), graph_bandwidth)
        print(f"Graph has bandwidth {graph_bandwidth}.")
        a = [5]; a[4]
...
def load_data(file_name):
    ...
    from matplotlib import cm
    from matplotlib import pyplot as plt
    all_vertices = misc.NameGenerator(name_type='number').generate(30)
    ...
    # ERROR PLOT 2 ----------------------------------------------------------------------
    # plt.subplots(figsize=(4.2, 3.75), dpi=1000)
    plt.plot(np.sum([ekf_residuals[v] for v in all_vertices], 0)/MONTE_CARLO_TRIALS, color='black', label="EKF")
    plt.plot(np.sum([lbekf_residuals[v] for v in all_vertices], 0)/MONTE_CARLO_TRIALS,
             color='black', linestyle='dashed', label="LB-EKF+VR")
    plt.plot(np.sum([lbekf_shuffled_residuals[new_labels[v]] for v in all_vertices], 0)/MONTE_CARLO_TRIALS,
             color='black', linestyle='dotted', label="LB-EKF")
    plt.ylabel(r'Total Mean Squared Error (MSE)')
    plt.xlabel(r'Timestep')
    plt.xlim([0, 100])
